...
class CucumberRerunReprotParser(object):
	...
	def __write_json(self, json, new_json_report):
		...
		try:
			...
			raise Exception('Cannot write the new report {} with content {}'.format(new_json_report))
	...
	def get_passed_scenario_from_rerun_report(self):
		...
		logging.info('Get scenarios and results maps from {}'.format(self.rerun_json_report))
		...
		if scenarios_num > 0 :
			...
			if len(scenarios_and_results)==scenarios_num:
				...
				raise Exception('Scenarios total num is changed! Something should be wrong!')
			...
			logging.info("There is no content in {}".format(self.rerun_json_report))
			return {}
	...
	def refresh_report_with_rerun_passed_scenarios(self, scenarios_and_results):
		...
		"""
		if len(scenarios_and_results) <= 0:
			logging.info('No rerun passed scenarios. No refreshment is needed!')
			return
		else:
			logging.info("Update the report {} with rerun results {}".format(self.original_json_report, scenarios_and_results))
			logging.info("Get the data from {}".format(self.original_json_report))
			raw_data = self.__read_json(self.original_json_report)

			cases_num = len(raw_data)
			if cases_num > 0:
				logging.info("There are {} scenarios totally.".format(str(cases_num)))
				logging.info("Rewrite the results of {} scenarios".format(str(len(scenarios_and_results))))
				...
				for item in scenarios_and_results:
					logging.info('Appending the passed scenario:{}'.format(item))
					raw_data.append(item)

				if len(raw_data)==cases_num:
					logging.info("Complete the result refresh")
					logging.info("Now there are {} cases passed".format(str(cases_num)))
					return json.dumps(raw_data)
				else:
					raise Exception('Scenarios total num is changed! Something should be wrong!')
			else:
				logging.info("There is no {} or content in the file.".format(self.original_json_report))
				return

	def write_new_report(self, json):
	   logging.info('Write the new json report {}'.format(self.new_json_report))
	   if json is not None:
	       self.__write_json(json, self.new_json_report)
	       logging.info('Complete to generate the new report {}'.format(self.new_json_report))
	   else:
...
if __name__ == '__main__':
    ...
    new_report_json = parser.refresh_report_with_rerun_passed_scenarios(passed_scenarios)
